...
# pcasl         <- antsImageRead("../data/109896_20100617_0005_ep2d_se_pcasl_PHC_1500ms.nii.gz", 4)
casl          <- antsImageRead("../data/CASL.nii.gz", 4)
t1            <- antsImageRead("../data/t1.nii.gz", 3)
# t1            <- antsImageRead("../data/109896_20100617_mprage_t1.nii.gz", 3)
...
prior.csf     <- antsImageRead("../data/template/Labels/T1template_prior_01.nii.gz", 3)
prior.gm      <- antsImageRead("../data/template/Labels/T1template_prior_02.nii.gz", 3)
prior.wm      <- antsImageRead("../data/template/Labels/T1template_prior_03.nii.gz", 3)
@
...
<<preprocess,results="hide">>=
mydir <- paste(tempdir(),"/Z", sep='')
t1.n4 <- abpN4(img=t1,  intensityTruncation=c( 0.025, 0.999, 256 ) )
t1.brainx <- abpBrainExtraction(img=t1.n4, tem=template, temmask = mask.cerebrum)
...
#  fixed=templatebrain, moving=t1.brainx$brain, "Affine", mydir))
...
for ( its in 1:3 ) {
  segs <- Atropos(d = 3, a = brain, m = "[0.1,1x1x1]", c = "[5,0]",
    i = probabilityimages, x =  t1.brainx$bmask , priorweight = 0.5 )
...
segs <- Atropos(d = 3, a = brain, m = "[0.1,1x1x1]", c = "[25,0]",
    i = probabilityimages, x =  t1.brainx$bmask , priorweight = 0.33 )
...
transform.asl2t1 <- suppressMessages(antsRegistration(t1.brainx$brain, avgpcasl, "Rigid", mydir))
...
<<getAAL>>=
aal      <- antsImageRead("../data/template/Labels/aal.nii.gz", 3)
aal2t1<-antsApplyTransforms( fixed=t1,moving=aal,interpolator="NearestNeighbor",t=t1.brainx$invtransforms )
