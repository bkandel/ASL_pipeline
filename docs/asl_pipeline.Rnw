...
t1.brain <- abpBrainExtraction(img=t1.n4, tem=template, temmask = mask.cerebrum)

transform.t12template <- suppressMessages(antsRegistration(
  fixed=template, moving=t1.n4, "Affine", mydir))
...
transform.asl2t1 <- suppressMessages(antsRegistration(t1, m0, "Rigid", mydir))
