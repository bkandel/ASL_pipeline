...
rootdir<-"./data/"
aal           <- antsImageRead(paste(rootdir,"template/Labels/aal.nii.gz",sep=''), 3)
...
aal2asl <- antsApplyTransforms( moving=aal, fixed=avgpcasl, interpolator="NearestNeighbor",
                      t=concatenatedtx )
@
...
mynetwork<-filterfMRIforNetworkAnalysis( pcasl.processing$aslTimeSeries , tr=4, mask=mask ,cbfnetwork = "ASLCBF", labels = aal2asl , graphdensity = 0.05 )
plot( bb$graph$walktrapcomm, mynetwork$graph$mygraph )
@
